...
export default {
  ...
  // 新增任务
  addTaskDetail: function(obj, data) {
    ...
    http.wyPost(url, data).then(
      ...
      response => {
        const toastVnode = {
          ...
          variant: "success",
          title: "操作成功",
          content: `${obj.deleteData.projectName}-${obj.deleteData.taskName}`
        };
        obj.$WYToast.Toast(obj, toastVnode);
        obj.deleteData = {
          id: "",
        ...
        const nextParams = {
          keyword: obj.filterForm.keyWord,
          scriptStatus: obj.filterForm.scriptStatus,
          isScript: obj.filterForm.isScript
        };
        http.wyPost(nextUrl, nextParams).then(nextResponse => {
          ...
          obj.$WYHttp.task.getTaskList(obj, lastParams);
        });
      },
      // eslint-disable-next-line no-unused-vars
      err => {
  ...
  // 获取任务详情
  getTaskDetail: function(obj, data) {
    const url = "/AdminApi/task_info";
    http.wyPost(url, data).then(
      response => {
  ...
  // 获取任务详情
  getTaskDetailUpdate: function(obj, data) {
    const url = "/AdminApi/task_info";
    http.wyPost(url, data).then(
      response => {
        obj.detailData = response.data;
  ...
  // 获取探测结果数据
  getTaskResultList: function(obj, data) {
    const url = "/AdminApi/get_detect_list";
    http.wyPost(url, data).then(
      response => {
        obj.items = response.data.list;
        obj.total = response.data.count;
