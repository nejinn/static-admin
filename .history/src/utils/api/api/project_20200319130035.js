...
export default {
  ...
  getProjectList: function(obj, data) {
    ...
    http.wyPost(url, data).then(
      response => {
        ...
        // obj.$store.commit("setProjectManageDataPerPageSize", obj.perPageSize);
  ...
  uploadProjectData: function(obj, data) {
    ...
    http.wyPost(url, data).then(
      response => {
        const file_url = response.data[0].file_url;
        obj.detailData.accessory.unshift(file_url);
        console.log(obj.detailData.accessory);
