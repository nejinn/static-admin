            ...
            <b-modal id="modal-delete" hide-header>
              <template>
                <p class="my-4">
                  您正在删除
                  <b>
                    {{ deleteData.projectName }}-{{ deleteData.taskName }}
                  </b>
                </p>
              </template>
...
<script>
import taskLogTable from "./_taskLogTable.js";
export default {
  name: "taskManage",
  data() {
    return {
      cardHeaderTitle: taskLogTable.name,
      cardBodyIsOpen: true,
      fields: taskLogTable.fields,
      items: null,
      selected: [],
  ...
  methods: {
    ...
    goView(row) {
      this.$router.push({
        name: "TaskView",
        params: { id: row.item.taskId, name: row.item.taskName }
      });
    },
    goEditor(row) {
      this.$router.push({
  ...
  mounted() {
    ...
    this.$WYHttp.task.getTaskLogList(obj, params);
  },
  created() {
    this.currentPage = 1;
    this.perPageSize = 10;
