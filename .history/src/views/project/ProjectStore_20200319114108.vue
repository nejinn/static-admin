...
export default {
  ...
  methods: {
    postFile() {
      console.log(this.accessory[0]);
      // const formData = new FormData();
      // formData.append("fileData", this.accessory[0]);
      // console.log(formData);
      let param = new FormData();
      param.append("name", "wiiiiiinney");
      //通过append向form对象添加数据
      param.append("file", this.accessory[0]);
      //FormData私有类对象，访问不到，可以通过get判断值是否传进去
      console.log(param.get("file"));
      const params = {
        accessory: this.accessory[0]
      };
      const obj = this;
      this.$WYHttp.project.uploadProjectData(obj, params);
    },
    goPrev() {
      this.$router.go(-1);
    },
